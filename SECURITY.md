...
3. The access token is read once per app process, reused in memory for the periodic fetches, and used as `Authorization: Bearer …`. Claude Code renews its own login several times a day and rewrites that Keychain item, which invalidates the token held in memory — and, because the rewrite re-stamps the item's partition list, also revokes this app's permission to read it. Reading the replacement therefore always costs a macOS dialog, so it is done only when you ask: at app launch, on connect, and on an explicit refresh or reauthorization. A `401` during an automatic refresh discards the token, freezes the display on the last known values and stops the poll loop — it never opens the Keychain. Any single refresh you trigger reads the Keychain at most once and makes at most two requests.
