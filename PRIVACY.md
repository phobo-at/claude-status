...
- Claude Code OAuth access token: after user authorization, read once per app launch from the local macOS Keychain, reused in memory for further fetches, and used exclusively for the authenticated HTTPS request to Anthropic. It is read again only when you ask for it — Claude Code renews its own login several times a day, and macOS revokes the app's Keychain permission each time it does, so the replacement is fetched on an explicit action rather than in the background.
