...
- **Token rotation, and why the keychain is gated on intent.** Claude Code renews its OAuth token several times a day (measured: roughly every 10 h) and rewrites the keychain item through `/usr/bin/security`. That rewrite does **not** just invalidate the token this app is holding — it also re-stamps the item's *partition list* with the writing tool's partition alone, dropping the `teamid:` entry macOS recorded when the user chose "Always Allow". Measured on the real item: the trusted-application list survives the rewrite (each grant appends another `/Applications/ClaudeStatus.app` entry, which is why duplicates pile up), the partition entry does not. Consequence: **every** re-read after a renewal raises a macOS dialog, and no code here can prevent that — restoring the partition entry needs the login keychain password. So a `401` on the **in-memory** token re-reads the keychain only when `userInitiated` is true; an automatic refresh throws `ReauthorizationRequired` instead, which freezes the display on the last snapshot as `reauthorizationRequired` and stops the poll loop (`isWaitingForUser`) so no further request spends the shared rate-limit budget on a token that is gone. `CredentialError.accessDenied` — a dismissed dialog — lands in the same state rather than in a backoff loop. A `401` on a **freshly read** token is a real login problem: it surfaces as `authenticationRequired` and likewise stops automatic keychain reads. That bounds a user-initiated refresh at two requests and one keychain read, and an automatic one at zero keychain reads, always.
