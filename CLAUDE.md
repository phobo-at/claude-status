...
- **`ViewModels/UsageStore.swift`** — `@MainActor` `ObservableObject`, the central state machine. Owns the `UsageDisplayState` enum (`disconnected` / `loading` / `current` / `stale` / `authenticationRequired` / `failed`), the 5-min polling task (`automaticRefreshInterval`), and exponential backoff on failure (`[60,120,300,900]`, honoring `Retry-After`). Automatic triggers — the poll loop, popover-open, and wake-from-sleep — all funnel through `refreshAutomaticallyIfStale()`, which skips the fetch while the snapshot is younger than `minimumAutomaticRefreshAge` (2 min); only explicit user actions force a refresh. Injects `CredentialProviding`, `UsageFetching`, `SnapshotCaching`, `UserDefaults`, and a `now` closure — all tests construct it with fakes and a fixed clock.
